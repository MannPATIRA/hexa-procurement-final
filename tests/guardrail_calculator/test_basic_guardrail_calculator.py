...
def test_calculate_guardrails_returns_guardrail_store():
    ...
    result = calculator.calculate_guardrails(supplier_state_store, sales_forecast)
...
def test_calculate_guardrails_has_correct_period():
    """Test that guardrail valid period matches sales forecast period."""
    calculator = BasicGuardrailCalculator()
    erp_fetcher = MockERPDataFetcher()
    crm_fetcher = MockCRMDataFetcher()
    ...
    # All guardrails should have same period as sales forecast
    for guardrail in result.items:
        assert guardrail.valid_period_start == sales_forecast.forecast_period_start
        assert guardrail.valid_period_end == sales_forecast.forecast_period_end


def test_calculate_guardrails_uses_lead_times():
    """Test that guardrails are calculated using lead times from supplier state."""
    calculator = BasicGuardrailCalculator()
    erp_fetcher = MockERPDataFetcher()
    crm_fetcher = MockCRMDataFetcher()
    ...
    # Find guardrail for PROD-001 (which has lead time of 7 days from mock data)
    prod_001_guardrail = next((g for g in result.items if g.product_id == "PROD-001"), None)
    assert prod_001_guardrail is not None
    # Reorder point and safety stock should be > 0 if lead time is used
    assert prod_001_guardrail.reorder_point > 0
    assert prod_001_guardrail.safety_stock > 0
